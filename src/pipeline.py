...
class DiagnosticPipeline:
    ...
    async def run(
        ...
        # If historical data exists but the metric was previously zero,
        # and the incident rate is now positive, this is a new anomaly.
        if baseline > 0:
        ...
        ranked = sorted(
            candidate_dims,
            key=lambda item: item[1]["total_excess"],
            reverse=True,
        )
        ...
        for other_dim in DIMENSIONS:
            ...
            # Avoid drilling into a dimension already fixed by scope.
            if (
                other_dim == "region"
                and region_hint
            ):
